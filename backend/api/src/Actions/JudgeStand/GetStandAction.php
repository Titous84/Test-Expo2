...
use App\Services\StandService_ss;
...
/**
 * Action qui permet d'obtenir les stands
 * @author Souleymane Soumaré
 */
class GetStandAction
{
    private $standService;
	private $tokenService;

	public function __construct(StandService_ss $standService, TokenService $tokenService)
	{
	...
	public function __invoke(Request $request, Response $response, $args): ResponseInterface
	{
		...
		$result = $this->standService->showingStands();
        $response->getBody()->write($result->to_json());
		return $response->withStatus($result->get_http_code());
	}
}